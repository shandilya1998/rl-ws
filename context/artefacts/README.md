...
| `2026-07-28_06-37-24-play/` | 11 dashboard plots, `base_position`, `commands`, `feet_clearance`, `feet_distances`, `joint_accelerations`, `joint_position`, `joint_powers`, `joint_torques`, `joint_velocities`, `rewards`, `torque_vs_joint_velocities`, plus `play_video.mp4` and two contact sheets extracted from it, `video_frames_episode` and `video_frames_one_cycle` | Play evaluation of `sd_brs1_flat/2026-07-28_06-37-24` | Certain on two independent grounds, the plot legends carry the run label and the video is a byte identical md5 match against `videos/play/42/rl-video-step-0.mp4` in that run's own directory |

The fifth directory is cited as a complete set by the twentieth pass of `../brs_gait.md` and by section 3 of `../../plans/GAIT_EFFICIENCY_PLAN.md`, which reason about all eleven plots together as the instruments of one evaluation. Two caveats attach to it. Every panel labelled R shows the LEFT joint or foot and every panel labelled L shows the right, because the dashboard's `JOINT_NAMES` and `FEET_NAMES` tables are transposed against the true articulation order, which the twentieth pass proves and which no aggregate conclusion depends on. And the two contact sheets were extracted from `play_video.mp4` for this analysis rather than exported by the dashboard, the first sampling the whole thirty second episode at two and a half second intervals and the second one gait cycle at sixty millisecond intervals, both cropped and upscaled about the tracked robot.

Three of the first four directories are cited as complete sets rather than file by file. The fourteen panels of `plots/` are enumerated in the file map of `../knowledge_base.md` and reasoned about individually in section 4 of `../../plans/COPT_INVESTIGATION_PLAN.md`. The eight plots of `plots_latest_play/` are cited as the eight diagnostic plots of that evaluation in both documents that use them. The ten panels of `plots-latest/` carry three individual embeds in `../../plans/LIPM_REWARD.md`, namely `reward7`, `loss2`, and `metrics`, and the remaining seven supply the per component reward bands and the explained variance trace that the same passage and the LIPM section of `../knowledge_base.md` reason about collectively, so the directory is retained whole.
