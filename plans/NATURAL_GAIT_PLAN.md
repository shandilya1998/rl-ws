...
Note added 2026-07-31. A subsequent audit of every reader of `net_forces_w_history` found this correction partially reverted in the working tree, `keep_ankle_pitch_zero_in_air` having lost the argument and its defect note while its body was rewritten to read indices zero and one unconditionally. That form gives the right answer for SD_BRS1 but reaches the TRON1 SF caller at `cfg/SF/limx_base_env_cfg.py` line 1148 as well, which no run asked for, so the argument was restored exactly as described above and the explicit zero set at `cfg/SF/brs_base_env_cfg.py` line 700. The audit further confirms that no other member of the family is outstanding. `feet_slide` and both foot clearance rewards take a maximum over the whole history axis and are indexing agnostic, `no_contact` and `knee_flexion_in_swing_v2` read index zero already, and `knee_flexion_in_swing` v1 retains the stale pair only because it is superseded and unwired.
